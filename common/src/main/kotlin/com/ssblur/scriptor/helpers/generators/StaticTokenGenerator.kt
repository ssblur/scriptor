...
class StaticTokenGenerator(obj: JsonObject?) : TokenGenerator() {
    ...
    override fun generateToken(key: String, parameters: JsonObject?): String {
        ...
        if (!parameters.has("token"))
            throw MissingRequiredElementException("token", "The word \"$key\" must have a token defined because it uses a static token generator.")
        ...
        if (usedTokens.containsKey(token))
            when (this.parameters.collisionStrategy) {
                ...
                CollisionStrategy.FALLBACK -> return getGenerator(TokenGeneratorRegistry.defaultGenerator)!!.generateToken(key, null)
                null -> {}
            }
