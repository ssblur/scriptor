...
object ScriptorDataComponents {
    ...
    private fun <T> register(name: String, customizer: Consumer<DataComponentType.Builder<T>>) = ScriptorMod.registerDataComponent(name, customizer)
    private fun registerLong(name: String) = ScriptorMod.registerDataComponent(name) { it.persistent(Codec.LONG).networkSynchronized(ByteBufCodecs.VAR_LONG) }
    private fun registerBool(name: String) = ScriptorMod.registerDataComponent(name) { it.persistent(Codec.BOOL).networkSynchronized(ByteBufCodecs.BOOL) }
    private fun registerInt(name: String) = ScriptorMod.registerDataComponent(name) { it.persistent(Codec.INT).networkSynchronized(ByteBufCodecs.INT) }
    private fun registerString(name: String) = ScriptorMod.registerDataComponent(name) { it.persistent(Codec.STRING).networkSynchronized(ByteBufCodecs.STRING_UTF8) }
