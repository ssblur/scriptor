...
object WordRegistry {
    ...
    init {
        ...
        actionRegistry.forEach{ (key, value) ->
            val cost = value.cost()
            val type = Word.COSTTYPE.valueOf(config.get("action.$key.type", cost.type.toString(), "Cost type to use for $key"))
        ...
        subjectRegistry.forEach{ (key, value) ->
            val cost = value.cost()
            val type = Word.COSTTYPE.valueOf(config.get("subject.$key.type", cost.type.toString(), "Cost type to use for $key"))
        ...
        descriptorRegistry.forEach{ (key, value) ->
            val cost = value.cost()
            val type = Word.COSTTYPE.valueOf(config.get("descriptor.$key.type", cost.type.toString(), "Cost type to use for $key"))
